...
RSpec.describe Pair, type: :model do
  ...
  it { should have_many(:orderbook_asks).dependent(:destroy) }
  it { should have_many(:orderbook_bids).dependent(:destroy) }
  ...
  describe 'daily check for new pairs' do
    ...
    let(:orderbook_asks) { create_list(:orderbook_ask, 6, pair: pair) }
    let(:orderbook_bids) { create_list(:orderbook_bid, 6, pair: pair) }
    ...
    it 'will check for new pair' do
      ...
      expect(Pair).to receive('check_for_orderbook').and_return(Array)
      Pair.check_shedule
    end
  end
end
